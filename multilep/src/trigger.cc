...
/*
 * Here we need to rewrite some stuff if we want to keep our tuples a bit flexible
 * If we don't store the triggers separately, we can't use it for trigger efficiency measurements 
 * Implementation in the Majorana package looks better
 */
...
bool multilep::metFiltersFlagged(const edm::Event& iEvent){
  ...
  iEvent.getByToken(recoResultsToken, recoResults);
  ...
  edm::Handle<bool> badPFMuonFilter;   iEvent.getByToken(badPFMuonFilterToken, badPFMuonFilter);
  edm::Handle<bool> badChCandFilter;   iEvent.getByToken(badChCandFilterToken, badChCandFilter);
