...
JEC::JEC(const std::string& JECPath, bool dataSample, bool fall17Sample, double forMETJetPtLevel):
    path(JECPath), isData(dataSample), is2017(fall17Sample) , currentJEC("none"), jetPtThreshold(forMETJetPtLevel) {}
...
std::pair<double, double> JEC::getMETCorrectionPxPy(double rawPt, double rawEta, double rawMuonSubtractedPt, double phi, double emf, double rho, double area){
    ...
    //if(emf > 0.9 || fullcorrpt < 15 || (fullcorrpt < 75 && fabs(rawEta) > 2.7 && fabs(rawEta) < 3) || (fabs(rawEta) > 9.9) ) return {0., 0.}; // skip jets with EMF > 0.9
    if(emf > 0.9 || fullcorrpt < jetPtThreshold || (fabs(rawEta) > 9.9) ) return {0., 0.}; // skip jets with EMF > 0.9
