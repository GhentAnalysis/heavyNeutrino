...
if len(jobsToResubmit) > 0:
  ...
  else:             os.system("crab resubmit --jobids=" + jobsToCrabList(jobsToResubmit) + (' --maxmemory=4000' if raiseMemoryLimit else '') + " --siteblacklist=T2_US_Wisconsin,T2_US_Vanderbilt,T2_CH_CERN,T2_BE_IIHE,T2_IT_Rome,T2_BR_SPRACE,T2_FR_CCIN2P3")
